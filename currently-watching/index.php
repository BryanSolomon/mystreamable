...
$querysearchfull = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1";
...
if(isset($_POST['button-search'])){
    ...
    $querysearch = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE name='".$title."' AND userID='".$_SESSION['userID']."' AND listID=1";
...
if(isset($_POST['button-filter'])){
    ...
    $trigger = true;
    $service = $_POST['filter-streamingService'];

    if($service != "All"){
        if($_POST['filter-itemType'] == "Movie"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1 AND streamingService='".$service."' AND type=1";
        }
        else if($_POST['filter-itemType'] == "TV Show"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1 AND streamingService='".$service."' AND type=2";
        }
        else if($_POST['filter-itemType'] == "All"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1 AND streamingService='".$service."'";
        }
    }
    else{
        if($_POST['filter-itemType'] == "Movie"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1 AND type=1";
        }
        else if($_POST['filter-itemType'] == "TV Show"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1 AND type=2";
        }
        else if($_POST['filter-itemType'] == "All"){
            $queryfilter = "SELECT name, objectID, season, episode, streamingService, type FROM listItems WHERE userID='".$_SESSION['userID']."' AND listID=1";
        }
    }
...
<head>
    <title>Currently Watching | Streamable</title>
    <meta charset="utf-8" />
    ...
    <div class="w-100 text-white bg-dark hero d-flex flex-column mb-3 hero-size">
        <div class="w-100 h-100 darken bg-blur d-flex">
            <div class="container mt-auto mb-auto">
                <h1>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-collection-play hero-icon" viewBox="0 0 16 16">
                ...
                <div class="col-xl-7 mb-3">
                        ...
                        <div class="col-md-9">
                            ...
                            <p>
                                <?php 
                                    echo "<p>";
                                    if ($row['streamingService'] != "" && $row['type'] == 2 && $row['season'] != 0 && $row['episode'] != 0) { 
                                        echo "Watching on " . $row['streamingService'] . " | " . "Season " . $row['season'] . ", Episode " . $row['episode']; 
                                    } else {
                                        if ($row['streamingService'] != "") echo "Watching on " . $row['streamingService'];
                                        if ($row['type'] == 2 && $row['season'] != 0 && $row['episode'] != 0){
                                            echo "Season " . $row['season'] . ", Episode " . $row['episode'];
                                        }
                                    }
                                    echo "</p>";
                            ...
                            <?php
                                ...
                                if(mysqli_num_rows($result) == 0) {
                                    ?>
                                    <a id="<?php echo $obj?>-fav" class="btn btn-light mb-1 me-1" onclick="update(<?php echo $obj ?>,<?php echo $type?>,1,`<?php echo $obj?>-fav`,1);">
                                ...
                                // Mark complete
                                $sql = "SELECT * FROM listItems WHERE userID = $userID AND objectID = '" . $obj . "' AND listID = 1";
                                $result = mysqli_query($conn, $sql);
                                    ?>
                                    <a id="<?php echo $obj?>-mark" class="btn btn-light mb-1 me-1" onclick="update(<?php echo $obj ?>,<?php echo $type?>,5,`<?php echo $obj?>-mark`);">
                                    <!-- <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-check btn-icons" viewBox="0 0 16 16">
                                        <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"/>
                                    </svg> -->
                                    <!-- <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-check2 btn-icons" viewBox="0 0 16 16">
                                        <path d="M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z"/>
                                    </svg> -->
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-check-circle btn-icons" viewBox="0 0 16 16">
