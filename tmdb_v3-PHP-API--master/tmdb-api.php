...
class TMDB {
    ...
    /**
	* Get list of flatrate providers of Movie in US region
	*
	* @param int $idMoview The Movie id
	* @return array of providers
	*/
    public function getMovieProvider($idMovie){
        $results = $this->_call('movie/' . $idMovie . '/watch/providers');
        $providers = array();
        foreach($results['results']['US']['flatrate'] as $data) {
	...
	/**
	* Get list of flatrate providers of TVShow in US region
	*
	* @param int $idTVShow The TVShow id
	* @return array of providers
	*/
    public function getTVShowProvider($idTVShow){
        $results = $this->_call('tv/' . $idTVShow . '/watch/providers');
        $providers = array();
        foreach($results['results']['US']['flatrate'] as $data) {
            $providers[] = $data['provider_name'];
        }
